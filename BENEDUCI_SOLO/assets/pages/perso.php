...
<h1 class="homecatname">Personnages</h1>

    <div class="postdivContent">
        <?php
            // selection des post de la catégorie "personnages"
            $bdd=connectDb();
            $p = $bdd->prepare('SELECT count(*) as total FROM posts where id_cat = 2');
            $p->execute();
            $okpost = $p->fetch();
            if($okpost['total'] != 0){ // si il y a des posts dans cette catégorie
                $query = $bdd->prepare('SELECT * FROM posts where id_cat = 2');
                $query->execute();
