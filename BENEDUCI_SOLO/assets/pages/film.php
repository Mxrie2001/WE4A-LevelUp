...
<h1 class="homecatname">Film</h1>

    <div class="postdivContent">
        <?php
            // selection des post de la catégorie "film"
            $bdd=connectDb();
            $p = $bdd->prepare('SELECT count(*) as total FROM posts where id_cat = 5');
            $p->execute();
            $okpost = $p->fetch();
            if($okpost['total'] != 0){ // si il y a des posts dans cette catégorie
                $query = $bdd->prepare('SELECT * FROM posts where id_cat = 5');
            $query->execute();
